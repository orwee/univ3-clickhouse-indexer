...
-- (the pool that swapped there is the one that moved first) and CLOSES at the first swap after
-- it with |gap| <= fee_bps (the pool that swapped there is the one that closed it). Duration is
...
WITH
    lo AS
    (
        SELECT block_number AS blk, block_number * 100000 + log_index AS pos, toUInt8(1) AS pair,
    ...
    events AS
    (
        SELECT lo.blk AS blk, lo.pos AS pos, 'lo' AS mover, 2e4 * log(lo.sp / hi.sp) AS gap_bps
        FROM lo ASOF JOIN hi ON lo.pair = hi.pair AND lo.pos > hi.pos
        UNION ALL
        SELECT hi.blk AS blk, hi.pos AS pos, 'hi' AS mover, 2e4 * log(lo.sp / hi.sp) AS gap_bps
        FROM hi ASOF JOIN lo ON hi.pair = lo.pair AND hi.pos > lo.pos
    ),
    marked AS
    (
        SELECT blk, pos, mover, gap_bps,
    ...
    runs AS
    (
        SELECT blk, pos, mover, gap_bps, is_open,
    ...
    (
        SELECT run_id, any(is_open) AS open, argMin(blk, pos) AS start_blk,
               argMin(mover, pos) AS opened_by, count() AS swaps_in_run, max(abs(gap_bps)) AS peak_bps
        FROM runs
        GROUP BY run_id
    ),
    episodes AS
    (
        SELECT run_id, open, start_blk, opened_by, swaps_in_run, peak_bps,
