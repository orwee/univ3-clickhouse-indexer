...
| Cannot push to `main` or force-push | **Yes, since 2026-09-21.** The `protect-main` ruleset is `enforcement: active` on the default branch: a pull request is required and non-fast-forward pushes are refused, with no bypass actors. It does not forbid deleting `main`, and it requires no approving review, so a pull request can still be merged by its own author. The global `pre-push` hook still blocks both locally, and `--no-verify` still skips that one | 2026-09-21, read-only and checked twice: `GET /repos/{owner}/{repo}/rulesets?includes_parents=true` returns one ruleset with `"enforcement":"active"`, and `GET /repos/{owner}/{repo}/rules/branches/main` returns `non_fast_forward` and `pull_request`. Classic branch protection still cannot be read: the token gets HTTP 403 |
