...
| Cannot push to `main` or force-push | **No barrier on GitHub's side today.** A global `pre-push` hook blocks both locally, but `--no-verify` skips it, so treat it as a convention and never use that flag. A ruleset named `protect-main` exists on the default branch — pull request required, non-fast-forward forbidden, no bypass actors — but it is still set to `enforcement: disabled`, so nothing on GitHub stops a direct push. Enabling it is one switch and would make this row a real barrier | 2026-09-21, read-only and checked twice: `GET /repos/{owner}/{repo}/rulesets?includes_parents=true` returns the one ruleset with `"enforcement":"disabled"`, and `GET /repos/{owner}/{repo}/rules/branches/main` returns `[]`, meaning no rule is in effect. Classic branch protection cannot be read at all: the token gets HTTP 403 |
